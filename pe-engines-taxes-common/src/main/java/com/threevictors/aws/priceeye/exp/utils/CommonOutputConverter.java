package com.threevictors.aws.priceeye.exp.utils;
...
public class CommonOutputConverter  implements Serializable {
    ...
    private List<RawLeg> buildOutboundLegList( PECommonOutput commonOutput, Map<String, String> airportToTimezoneMap ) {
        ...
        if ( outboundLegCount == 1 ) {
            ...
            int arrivalTime = buildTime(commonOutput.getOutbound_arrival_time());
            if(arrivalTime == 0) {
                calculateDateTimeWithDuration(rawLeg, true, airportToTimezoneMap);
            }
            else {
                rawLeg.setArriveTime(arrivalTime);
            }
        ...
        String [] fareClasses = commonOutput.getOutbound_fare_family().split("\\|");
    ...
    private List<RawLeg> buildInboundLegList( PECommonOutput commonOutput, Map<String, String> airportToTimezoneMap ) {
        ...
        if ( inboundLegCount == 1 ) {
            ...
            //Calculate arrival time of first leg
            int arrivalTime = buildTime(commonOutput.getInbound_arrival_time());
            if(arrivalTime == 0) {
                calculateDateTimeWithDuration(rawLeg, true, airportToTimezoneMap);
            }
            else {
    ...
    private long buildTimestamp( String commonOutputDate, String commonOutputTime ) {
        ...
        return dt.timezoneToMillis("America/Chicago");
    }
